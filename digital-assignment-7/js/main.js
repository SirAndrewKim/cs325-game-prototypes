...
function create() {
  ...
  cloud.create(520, 500, "cloud").setScale(1).refreshBody();

  var clouds = this.physics.add.image(100, 600, 'cloud')
  .setImmovable(true)
  .setVelocity(100, -100);
  ...
  this.physics.add.collider(clouds, player);
...
function update() {
  ...
  if (player.x < 0) {
    ...
    //Why does the character fall through the floor?
    player.x = 100;
  }
...
function collide(clouds, player){
  if(clouds.body.moves && clouds.touching.up && player.body.touching.down){
